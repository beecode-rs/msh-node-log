...
describe('logger - ConsoleLogger', () => {
    ...
    describe('constructor', () => {
        it('should set default log level', () => {
            chai_1.expect(defaultLogger['_logLevel']).to.equal(_1.LogLevel.ERROR);
        });
        it('should set log level passed through constructor', () => {
            const infoLogger = new _1.ConsoleLogger(_1.LogLevel.INFO);
            chai_1.expect(infoLogger['_logLevel']).to.equal(_1.LogLevel.INFO);
        });
    });
    describe('_logLevelToInt', () => {
        it('should return 0 for error', () => {
            chai_1.expect(defaultLogger['_logLevelToInt'](_1.LogLevel.ERROR)).to.equal(0);
        });
        it('should return 1 for warn', () => {
            chai_1.expect(defaultLogger['_logLevelToInt'](_1.LogLevel.WARN)).to.equal(1);
        });
        it('should return 2 for info', () => {
            chai_1.expect(defaultLogger['_logLevelToInt'](_1.LogLevel.INFO)).to.equal(2);
        });
        it('should return 3 for debug', () => {
            chai_1.expect(defaultLogger['_logLevelToInt'](_1.LogLevel.DEBUG)).to.equal(3);
        });
    ...
    describe('_logMessage', () => {
        ...
        it('should not log messages if shouldLog returns false', () => {
            ...
            logMessageLogger['_logMessage'](_1.LogLevel.ERROR, 'test message');
        ...
        it('should log only object message if no object passed', () => {
            ...
            logMessageLogger['_logMessage'](_1.LogLevel.ERROR, objMsg);
            sinon_1.assert.calledOnce(stub_logger_shouldLog);
            sinon_1.assert.calledWith(stub_logger_shouldLog, _1.LogLevel.ERROR);
            sinon_1.assert.calledOnce(stub_console_log);
        ...
        it('should call logger twice for message and object', () => {
            ...
            logMessageLogger['_logMessage'](_1.LogLevel.ERROR, 'test message', someObject);
            sinon_1.assert.calledOnce(stub_logger_shouldLog);
            sinon_1.assert.calledWith(stub_logger_shouldLog, _1.LogLevel.ERROR);
