...
describe('logger - ConsoleLogger', () => {
    ...
    describe('constructor', () => {
        it('should set default log level', () => {
            chai_1.expect(defaultLogger['_logLevel']).to.equal(log_level_type_1.LogLevelType.ERROR);
        });
        it('should set log level passed through constructor', () => {
            ...
            chai_1.expect(infoLogger['_logLevel']).to.equal(log_level_type_1.LogLevelType.INFO);
        });
        [
            ['error', log_level_type_1.LogLevelType.ERROR],
            ['warn', log_level_type_1.LogLevelType.WARN],
            ['info', log_level_type_1.LogLevelType.INFO],
            ['debug', log_level_type_1.LogLevelType.DEBUG],
            ['ERROR', log_level_type_1.LogLevelType.ERROR],
            ['WARN', log_level_type_1.LogLevelType.WARN],
            ['INFO', log_level_type_1.LogLevelType.INFO],
            ['DEBUG', log_level_type_1.LogLevelType.DEBUG],
        ].forEach(([testLogLevel, expectedLogLevel]) => {
            it(`should allow string values [${testLogLevel}]`, () => {
                const infoLogger = new console_logger_1.ConsoleLogger({ logLevel: testLogLevel });
                chai_1.expect(infoLogger['_logLevel']).to.equal(expectedLogLevel);
            });
        });
        it('should throw error if unknown log level passed', () => {
            try {
                new console_logger_1.ConsoleLogger({ logLevel: 'foo' });
                chai_1.expect.fail();
            }
            catch (e) {
        ...
        it('should throw error if object passed as log level', () => {
            try {
                new console_logger_1.ConsoleLogger({ logLevel: { test: 'foo' } });
                chai_1.expect.fail();
            }
            catch (e) {
                chai_1.expect(e.message).to.eq("Only string value allowed for log level. Allowed values ['error' | 'warn' | 'info' | 'debug']");
            }
        });
    });
    describe('_logLevelToInt', () => {
        it('should return 0 for error', () => {
            chai_1.expect(defaultLogger['_logLevelToInt'](log_level_type_1.LogLevelType.ERROR)).to.equal(0);
        });
        it('should return 1 for warn', () => {
            chai_1.expect(defaultLogger['_logLevelToInt'](log_level_type_1.LogLevelType.WARN)).to.equal(1);
        });
        it('should return 2 for info', () => {
            chai_1.expect(defaultLogger['_logLevelToInt'](log_level_type_1.LogLevelType.INFO)).to.equal(2);
        });
        it('should return 3 for debug', () => {
            chai_1.expect(defaultLogger['_logLevelToInt'](log_level_type_1.LogLevelType.DEBUG)).to.equal(3);
        });
    ...
    describe('_logMessage', () => {
        ...
        const consoleLogStrategy = new (console_log_strategy_test_1.mockConsoleLogStrategyFactory(sandbox))();
