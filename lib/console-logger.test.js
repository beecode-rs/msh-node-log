...
describe('ConsoleLogger', () => {
    ...
    describe('_logMessage', () => {
        ...
        it('should log only string message if no object passed', () => {
            ...
            sinon_1.assert.calledWith(consoleLogStrategy.log, { type: 'error', meta: undefined, prefix: undefined }, 'test message');
        });
    });
    ...
    describe('clone', () => {
        it('should just clone logger', () => {
            const toClone = new console_logger_1.ConsoleLogger({
                logLevel: log_level_type_1.LogLevelType.DEBUG,
                meta: { some: 'meta' },
                messagePrefix: 'somePrefix',
                consoleLogStrategy: new simple_console_log_1.SimpleConsoleLog(),
            });
            const clonedLogger = toClone.clone();
            (0, chai_1.expect)(clonedLogger['_meta']).to.deep.equal({ some: 'meta' });
            (0, chai_1.expect)(clonedLogger['_messagePrefix']).to.equal('somePrefix');
            (0, chai_1.expect)(clonedLogger['_logLevel']).to.equal(log_level_type_1.LogLevelType.DEBUG);
            (0, chai_1.expect)(clonedLogger['_consoleLogStrategy'] instanceof simple_console_log_1.SimpleConsoleLog).to.be.true;
            (0, chai_1.expect)(clonedLogger).not.to.equal(toClone);
        });
        it('should override all values', () => {
            ...
            const clonedLogger = toClone.clone({
                ...
                logLevel: log_level_type_1.LogLevelType.INFO,
            });
            (0, chai_1.expect)(clonedLogger['_meta']).to.deep.equal({ some: 'meta', additional: 'data' });
            (0, chai_1.expect)(clonedLogger['_messagePrefix']).to.equal('overridePrefix');
            (0, chai_1.expect)(clonedLogger['_logLevel']).to.equal(log_level_type_1.LogLevelType.INFO);
            (0, chai_1.expect)(clonedLogger['_consoleLogStrategy'] instanceof simple_console_log_1.SimpleConsoleLog).to.be.true;
        });
    });
});
